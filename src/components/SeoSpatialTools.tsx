...
export function SeoSpatialTools({ activeToolId }: SeoSpatialToolsProps) {
  ...
  if (activeToolId === 'social-meta-preview') {
    return <SocialMetaPreview onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'redirect-header-inspect') {
    return <RedirectHeaderInspect onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'dns-mx-txt') {
    return <DnsMxTxtLookup />;
  }
...
// ==========================================
function SocialMetaPreview({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
    ...
    <div className="space-y-6" id="social-meta-preview-container">
      <div className="pb-4 border-b border-white/5">
        <h2 className="text-xl font-semibold text-white flex items-center gap-2">
          <span className="p-1 px-2 text-xs font-mono bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 rounded">SEO</span>
          Social & Meta Tags Preview Generator
        </h2>
        <p className="text-sm text-gray-400">Generate fully search-engine optimized metadata. Preview live layouts in Google Search, Facebook, X (Twitter), and LinkedIn shares instantly.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Forms values */}
        <div className="bg-[#141414] border border-white/5 p-5 rounded-2xl space-y-4">
          <h3 className="text-xs font-bold font-mono text-cyan-400 tracking-wider uppercase mb-1">METADATA VALUES</h3>
          <div>
            <label className="text-xs font-mono text-gray-405 mb-1 block">PAGE TITLE (Recommend &lt; 60 chars):</label>
            <input
              type="text"
              className="w-full p-2.5 border border-white/10 rounded-lg text-xs bg-[#161616] text-white focus:outline-none focus:ring-1 focus:ring-cyan-500"
              value={title}
              onChange={(e) => setTitle(e.target.value)}
            />
            <span className="text-[10px] text-gray-500 mt-1 block font-mono text-right">{title.length} / 60 characters</span>
          </div>

          <div>
            <label className="text-xs font-mono text-gray-405 mb-1 block">META DESCRIPTION (Recommend &lt; 155 chars):</label>
            <textarea
              className="w-full h-20 p-2.5 border border-white/10 rounded-lg text-xs bg-[#161616] text-white focus:outline-none focus:ring-1 focus:ring-cyan-500"
              value={description}
              onChange={(e) => setDescription(e.target.value)}
            />
            <span className="text-[10px] text-gray-500 mt-1 block font-mono text-right">{description.length} / 155 characters</span>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-mono text-gray-405 mb-1 block">CANONICAL SITE URL:</label>
              <input
                type="text"
                className="w-full p-2.5 border border-white/10 rounded-lg text-xs bg-[#161616] text-white focus:outline-none"
                value={url}
                onChange={(e) => setUrl(e.target.value)}
              />
            </div>
            <div>
              <label className="text-xs font-mono text-gray-405 mb-1 block">SOCIAL SHARE IMAGE URL:</label>
              <input
                type="text"
                className="w-full p-2.5 border border-white/10 rounded-lg text-xs bg-[#161616] text-white focus:outline-none"
                value={imageUrl}
        ...
        <div className="space-y-4">
          <div className="flex border-b border-white/5 text-xs font-mono">
            {['google', 'facebook', 'twitter', 'linkedin'].map((tab) => (
              <button
                ...
                className={`p-2.5 px-4 capitalize border-b-2 cursor-pointer transition-colors ${previewTab === tab ? 'border-cyan-500 text-white font-bold' : 'border-transparent text-gray-500 hover:text-gray-300'}`}
              >
          ...
          <div className="p-5 rounded-2xl border border-white/10 bg-[#161616] min-h-[180px] flex flex-col justify-center select-none">
            {previewTab === 'google' && (
              <div className="space-y-1 font-sans">
                <div className="flex items-center gap-1.5 text-xs text-gray-400">
                  <span className="bg-white/10 p-1 px-1.5 rounded-full text-[10px] font-bold">G</span>
                  <div className="truncate">
                    <span className="text-gray-300 block leading-tight">{domain}</span>
                    <span className="text-gray-500 block leading-none text-[10px]">{url}</span>
                  </div>
                </div>
                <h4 className="text-sm font-semibold text-sky-400 hover:underline cursor-pointer leading-tight pt-1">
                  {title}
                </h4>
                <p className="text-xs text-gray-450 leading-relaxed pt-0.5">
                  {description}
                </p>
              </div>
            )}
      ...
      <div className="border border-white/10 rounded-2xl overflow-hidden bg-[#121212]">
        <div className="p-4 bg-white/5 border-b border-white/5 flex items-center justify-between">
          <span className="text-xs font-mono text-gray-400">READY-TO-COPY HEAD SECTIONS HEADER (&lt;head&gt;):</span>
          <button
        ...
        <pre className="p-5 font-mono text-xs text-cyan-300 leading-relaxed overflow-auto max-h-60 select-all bg-[#0e0e0e]/95">
          {getTagsCode()}
        </pre>
      </div>
    </div>
...
// ==========================================
function RedirectHeaderInspect({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  ...
  return (
    <div className="space-y-6" id="redirect-header-inspect-container">
      <div className="pb-4 border-b border-white/5">
        <h2 className="text-xl font-semibold text-white flex items-center gap-2">
          <span className="p-1 px-2 text-xs font-mono bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 rounded">HTTP</span>
          Redirect Hops & Security Headers Proxy Inspector
        </h2>
        <p className="text-sm text-gray-400">Traces redirectional paths (301/302), checks SSL configuration variables, cipher algorithms, and validates security response headers securely.</p>
      </div>
      ...
      <form onSubmit={handleInspect} className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            className="w-full p-2.5 pl-10 border border-white/10 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/20 focus:border-cyan-500 bg-[#161616] text-white placeholder-gray-600"
      ...
      {loading && (
        <div className="py-12 flex flex-col items-center justify-center text-center">
          ...
          <p className="text-sm text-gray-400">Performing server-side handshake proxy trace and analyzing security profiles...</p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in">
          ...
          <div className="space-y-4 lg:col-span-1">
            <div className="p-5 bg-[#141414] border border-white/5 rounded-2xl">
              <span className="text-xxs font-mono text-gray-500 block uppercase mb-3">Redirection Path Matrix</span>
                ...
                {inspectData.hops.map((hop: any, idx: number) => (
                  <div key={`hop-${idx}`} className="flex items-start gap-2 relative pl-4 before:absolute before:left-0 before:top-2 before:w-1.5 before:h-1.5 before:rounded-full before:bg-cyan-500">
                    <div className="text-xs font-mono space-y-0.5 max-w-xs break-all">
                      <p className="text-gray-300 select-all leading-tight">{hop.url}</p>
            ...
            {inspectData.ssl && (
              <div className="p-5 bg-cyan-950/10 border border-cyan-500/10 rounded-2xl space-y-3">
                ...
                <div className="space-y-2 text-xxs font-mono text-gray-300">
                  <p><span className="text-gray-500">Issued to Subject:</span> <br /><span className="text-gray-200">{inspectData.ssl.subject?.CN || 'Unknown'}</span></p>
                  <p><span className="text-gray-500">Certificate Issuer:</span> <br /><span className="text-gray-250">{inspectData.ssl.issuer?.O || inspectData.ssl.issuer?.CN || 'Secure Issuer'}</span></p>
                  <p><span className="text-gray-500">Connected Cipher:</span> <br /><span className="text-cyan-350">{inspectData.ssl.cipher}</span></p>
                  <p><span className="text-gray-500">Validity Ends:</span> <br /><span className="text-emerald-400">{inspectData.ssl.valid_to}</span></p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-2 space-y-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div className="p-3 bg-[#141414] border border-white/5 rounded-xl">
                <span className="text-xxs font-mono text-gray-500 block">HTTP STATUS CODE</span>
                <span className="text-lg font-mono font-bold text-emerald-400">{inspectData.status} {inspectData.statusText}</span>
              </div>
              <div className="p-3 bg-[#141414] border border-white/5 rounded-xl">
                <span className="text-xxs font-mono text-gray-500 block">RESPONSE TIME</span>
                <span className="text-lg font-mono font-bold text-white">{inspectData.responseTimeMs} ms</span>
              </div>
            ...
            <div className="border border-white/10 rounded-xl overflow-hidden bg-[#161616]">
              <div className="p-3.5 bg-white/5 border-b border-white/5 flex justify-between items-center text-xs font-mono">
                <span className="text-gray-450 font-bold uppercase">Dissected headers ({Object.keys(inspectData.headers).length} fields)</span>
                <button
                  onClick={() => onCopy(JSON.stringify(inspectData.headers, null, 2), 'headers-inspect')}
                  className="text-xxs text-gray-500 hover:text-white cursor-pointer"
                >
                  Copy JSON Stream
                </button>
              </div>

              <div className="divide-y divide-white/5 text-xxs font-mono max-h-80 overflow-auto">
                {Object.entries(inspectData.headers).map(([key, value]: any) => {
                  ...
                  return (
                    <div key={key} className={`flex p-2.5 transition-colors ${isSecurityHeader ? 'bg-cyan-500/5 hover:bg-cyan-500/10' : 'hover:bg-white/2'}`}>
                      <span className={`w-1/3 break-all select-all font-bold ${isSecurityHeader ? 'text-cyan-450 font-black' : 'text-gray-500'}`}>
                        {key}
                      </span>
                      <span className="w-2/3 break-all select-all font-medium text-gray-300">
                        {value}
                      </span>
                    </div>
                  );
  ...
  return (
    <div className="space-y-6" id="dns-mx-txt-container">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/5">
        <div>
          <h2 className="text-xl font-semibold text-white flex items-center gap-2">
            <span className="p-1 px-2 text-xs font-mono bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 rounded">DNS</span>
            Mail MX & TXT Verification Solver
          </h2>
          <p className="text-sm text-gray-400">Target custom domain verification streams, extract TXT site validations (SPF, DKIM, Google site checks), and evaluate MX values.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleQuery} className="flex gap-2 font-mono">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            className="w-full p-2.5 pl-10 border border-white/10 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-cyan-505 bg-[#161616] text-white placeholder-gray-600"
      ...
      {loading && (
        <div className="py-12 flex flex-col items-center justify-center text-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-cyan-500 mb-2" />
          <p className="text-sm text-gray-400 font-mono text-xs">Asking DNS authority servers for TXT, SPF, DKIM, and MX configurations...</p>
        </div>
      )}
      ...
      {dnsResults && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
          ...
          <div className="p-5 border border-white/10 rounded-2xl bg-[#161616] space-y-4">
            ...
            <div className="space-y-2 max-h-96 overflow-auto">
              {dnsResults.records.TXT && dnsResults.records.TXT.length > 0 ? (
                dnsResults.records.TXT.map((txt: string, i: number) => (
                  <div key={`txt-${i}`} className="p-3 rounded-lg bg-white/2 border border-white/5 space-y-1 font-mono text-xxs break-all select-all">
                    <p className="text-gray-100 font-bold leading-normal">{txt}</p>
                ...
                <p className="text-xs text-gray-500 italic p-4 text-center">No TXT domain configurations resolved.</p>
              )}
            </div>
          </div>
          ...
          <div className="p-5 border border-white/10 rounded-2xl bg-[#161616] space-y-4">
            ...
            <div className="space-y-2 max-h-96 overflow-auto">
              {dnsResults.records.MX && dnsResults.records.MX.length > 0 ? (
                ...
                <p className="text-xs text-gray-500 italic p-4 text-center">No MX server routing parameters registered.</p>
              )}
            </div>
          </div>
        </div>
