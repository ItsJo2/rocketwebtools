...
export function TextTools({ activeToolId }: { activeToolId: string }) {
  ...
  if (activeToolId === 'case-converter') {
    return <CaseConverter onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'word-counter') {
    return <WordCounter />;
  }
  if (activeToolId === 'markdown-preview') {
    return <MarkdownPreview onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'text-to-slug') {
    return <TextToSlug onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'lorem-ipsum') {
    return <LoremIpsumGenerator onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'remove-line-breaks') {
    return <RemoveLineBreaks onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'random-word') {
    return <RandomWordGenerator onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'privacy-policy') {
    return <PrivacyPolicyGenerator onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'terms-conditions') {
    return <TermsConditionsGenerator onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'disclaimer-gen') {
    return <DisclaimerGenerator onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'text-repeater') {
    return <TextRepeater onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'text-sorter') {
    return <TextSorter onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
  if (activeToolId === 'comma-separator') {
    return <CommaSeparator onCopy={handleCopy} copiedStatus={copiedStatus} />;
  }
...
// 1. CASE CONVERTER
function CaseConverter({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
    ...
    <div className="space-y-6" id="case-converter-container">
      <div className="pb-4 border-b border-white/5">
        <h2 className="text-xl font-semibold text-white flex items-center gap-2">
          <span className="p-1 px-2 text-xs font-mono bg-purple-500/10 text-purple-400 border border-purple-500/20 rounded">TEXT</span>
          Case Converter
        </h2>
        <p className="text-sm text-gray-400">Transform your paragraphs between lowercase, UPPERCASE, Title Case, Sentence Case, or code naming conventions.</p>
      </div>

      <div className="space-y-4">
        <div className="relative">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs font-mono text-gray-400">INPUT PARAGRAPH:</span>
            {input && (
              <button 
                type="button"
          ...
          <textarea
            className="w-full h-64 p-4 border border-white/10 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-[#161616] text-white placeholder-gray-600"
        ...
        <div className="flex flex-wrap gap-2">
          <button
            ...
            className="p-2 px-4 bg-white/5 text-gray-300 hover:bg-white/10 hover:text-white border border-white/10 rounded-lg text-sm font-medium transition-all cursor-pointer"
          >
            UPPERCASE
          </button>
          <button
    ...
    <div className="space-y-6" id="word-counter-container">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/5">
        <div>
          ...
          <p className="text-sm text-gray-400">Live statistics, reading duration estimations, and character summaries.</p>
        </div>
        <button 
          type="button"
          onClick={loadExample}
          className="p-1.5 px-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-medium text-gray-300 hover:text-white self-start cursor-pointer transition-colors"
        >
          Load Demo Text
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="bg-[#141414] border border-white/5 p-4 rounded-xl grid grid-cols-2 gap-4">
            <div>
              <span className="text-xs font-mono text-gray-400 uppercase">Words</span>
              <div className="text-2xl font-black text-white">{wordCount}</div>
            </div>
            <div>
              <span className="text-xs font-mono text-gray-400 uppercase">Characters</span>
              <div className="text-2xl font-black text-white">{charCount}</div>
            </div>
            <div>
              <span className="text-xs font-mono text-gray-400 uppercase">Lines</span>
              <div className="text-xl font-bold text-gray-200">{lineCount}</div>
            </div>
            <div>
              <span className="text-xs font-mono text-gray-400 uppercase">Letters Only</span>
              <div className="text-xl font-bold text-gray-200">{letterCount}</div>
            </div>
            <div>
              <span className="text-xs font-mono text-gray-400 uppercase">Numbers</span>
              <div className="text-xl font-bold text-gray-200">{numCount}</div>
            </div>
            <div>
              <span className="text-xs font-mono text-gray-400 uppercase">Whitespace</span>
              <div className="text-xl font-bold text-gray-200">{spaceCount}</div>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2">
          <label className="text-xs font-mono text-gray-400 mb-1.5 block">TYPE AND WE WILL RECALCULATE DENSITY:</label>
          <textarea
            className="w-full h-80 p-4 border border-white/10 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-[#161616] text-white placeholder-gray-600"
...
function MarkdownPreview({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  const [input, setInput] = useState('');
  
  // Self-written simple & safe Regex compiler for Markdown (headings, bolds, code, list items, etc.)
    ...
    <div className="space-y-6" id="markdown-preview-container">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-white/5">
        <div>
          ...
          <p className="text-sm text-gray-400">Live compile rich markdown markup lines into visual nested components.</p>
        </div>
        <button 
          type="button"
          onClick={loadDemo}
          className="p-1.5 px-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg text-xs font-medium text-gray-300 hover:text-white cursor-pointer transition-colors"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="flex flex-col">
          <label className="text-xs font-mono text-gray-400 mb-1.5 block">MARKDOWN WRITER:</label>
          <textarea
            className="w-full h-80 p-4 border border-white/10 rounded-lg text-sm font-mono focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 bg-[#161616] text-white placeholder-gray-600"
        ...
        {/* Live Preview / Compiled HTML */}
        <div className="flex flex-col">
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-xs font-mono text-gray-400">RENDERED PREVIEW:</span>
            {input && (
              <button 
                type="button"
          ...
          <div className="flex-1 p-5 border border-white/10 rounded-lg bg-[#161616] overflow-auto min-h-[320px] max-h-[320px]">
            {input ? (
              <div 
              ...
              <div className="h-full flex flex-col justify-center items-center text-center text-gray-500 text-xs">
...
function TextToSlug({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  const [input, setInput] = useState('');
  ...
  const generateSlug = (text: string) => {
    let result = text
      ...
      .replace(/[^\w\s-]/g, '')            // remove non-word characters except dashes/spaces
      .trim();
    ...
    // Replace double spaces & underscores with selected separator
  ...
  return (
    <div className="space-y-6" id="text-to-slug-container">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-sm text-gray-400">Convert standard text sentences into clean, SEO-friendly, web-safe URL slugs instantly.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          <h3 className="font-bold text-white border-b border-white/5 pb-2 mb-3">Slug Choices</h3>
          
          <div className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Word Separator:</label>
              <select
                value={separator}
                onChange={(e) => setSeparator(e.target.value)}
                className="w-full p-2.5 border border-white/10 rounded-lg text-sm bg-[#141414] text-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              >
            ...
            <label className="flex items-center gap-2 cursor-pointer font-medium text-gray-300 hover:text-white pt-2">
              <input
                type="checkbox"
                checked={lowercase}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div>
            <label className="text-xs font-mono text-gray-400 mb-1.5 block">RAW TEXT INPUT:</label>
            <input
              type="text"
              className="w-full p-3 border border-white/10 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20 bg-[#161616] text-white"
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-xs font-mono text-gray-400">GENERATED URL SLUG:</span>
              {slug && (
                <button
                  type="button"
            ...
            <div className="w-full p-3.5 border border-white/10 rounded-lg bg-[#161616] font-mono text-base text-purple-300 break-all select-all min-h-[48px]">
              {slug || <span className="text-gray-600 italic text-sm">Your slug will appear here as you type...</span>}
            </div>
          </div>
        </div>
      </div>
...
function LoremIpsumGenerator({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  ...
  const generateSentence = (addPreamble = false) => {
    let wordsCount = Math.floor(Math.random() * 8) + 6; // Sentence has 6-14 words
  ...
  const generateParagraph = (addPreamble = false) => {
    const totalSentences = Math.floor(Math.random() * 4) + 4; // 4 to 7 sentences
  ...
  return (
    <div className="space-y-6" id="lorem-ipsum-container">
      <div className="pb-4 border-b border-white/5">
        <h2 className="text-xl font-semibold text-white flex items-center gap-2">
          <span className="p-1 px-2 text-xs font-mono bg-purple-50/10 text-purple-400 border border-purple-500/20 rounded">TEXT</span>
          Lorem Ipsum Generator
        </h2>
        <p className="text-sm text-gray-400">Generate customizable blocks of standard pseudo-Latin dummy text placeholders for print, layout designs, and mockups.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          <h3 className="font-bold text-white border-b border-white/5 pb-2 mb-3 font-sans">Parameters</h3>
          
          <div className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Unit Count:</label>
              <input
                type="number"
                min="1"
                max="1000"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Generate By:</label>
              <div className="grid grid-cols-3 gap-1 p-1 bg-[#161616] border border-white/10 rounded-lg text-xs font-semibold">
                <button
                  type="button"
                  onClick={() => setType('paragraphs')}
                  className={`p-1.5 rounded transition-all text-center cursor-pointer ${type === 'paragraphs' ? 'bg-indigo-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}
                >
                  Paragraphs
                </button>
                <button
                  ...
                  className={`p-1.5 rounded transition-all text-center cursor-pointer ${type === 'sentences' ? 'bg-indigo-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}
                >
                  Sentences
                </button>
                <button
                  ...
                  className={`p-1.5 rounded transition-all text-center cursor-pointer ${type === 'words' ? 'bg-indigo-600 text-white shadow' : 'text-gray-400 hover:text-white'}`}
                >
                  Words
                </button>
              </div>
            ...
            <label className="flex items-center gap-2 cursor-pointer font-medium text-gray-300 hover:text-white">
              <input
                type="checkbox"
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <textarea
            readOnly
            className="w-full h-80 p-4 border border-white/10 rounded-lg text-sm bg-[#161616] text-white focus:outline-none placeholder-gray-600 focus:border-white/10 resize-y select-all"
            value={generatedText}
...
function RemoveLineBreaks({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  const [input, setInput] = useState('');
  ...
  return (
    <div className="space-y-6" id="remove-line-breaks-container">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-sm text-gray-400">Clean up text content copied from rigid layout engines like PDF viewers, converting broken lines into cohesive running paragraphs.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          <h3 className="font-bold text-white border-b border-white/5 pb-2 mb-3 font-sans">Formatting Choices</h3>
          
          <div className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1.5">Replace Breaks With:</label>
              <div className="grid grid-cols-2 gap-2 text-xs font-bold text-[#f5f5f5]">
                <button
                  type="button"
                  onClick={() => setReplacementMode('space')}
                  className={`p-2 rounded-lg border transition-all cursor-pointer ${replacementMode === 'space' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-white/5 text-gray-400 border-white/5 hover:text-white'}`}
                >
                  Space
                </button>
                <button
                  ...
                  className={`p-2 rounded-lg border transition-all cursor-pointer ${replacementMode === 'comma' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-white/5 text-gray-400 border-white/5 hover:text-white'}`}
                >
                  Comma (,)
                </button>
                <button
                  ...
                  className={`p-2 rounded-lg border transition-all cursor-pointer ${replacementMode === 'none' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-white/5 text-gray-400 border-white/5 hover:text-white'}`}
                >
                  No Separator
                </button>
                <button
                  ...
                  className={`p-2 rounded-lg border transition-all cursor-pointer ${replacementMode === 'custom' ? 'bg-indigo-600 text-white border-indigo-500' : 'bg-white/5 text-gray-400 border-white/5 hover:text-white'}`}
                >
                  Custom Char
                </button>
              </div>
            ...
            {replacementMode === 'custom' && (
              <div>
                <label className="text-xs font-semibold text-gray-400 block mb-1">Custom Character / String:</label>
                <input
                  type="text"
                  value={customChar}
                  onChange={(e) => setCustomChar(e.target.value)}
                  className="w-full p-2 border border-white/10 rounded-lg text-xs bg-[#141414] text-white focus:outline-none"
                  placeholder="e.g. | or - or \t"
                />
              </div>
            )}

            <label className="flex items-center gap-2 cursor-pointer font-medium text-gray-300 hover:text-white pt-2 border-t border-white/5">
              <input
                type="checkbox"
        ...
        <div className="lg:col-span-2 space-y-4">
          <div>
            <label className="text-xs font-mono text-gray-400 mb-1 block">SOURCE MULTI-LINE TEXT:</label>
            <textarea
              className="w-full h-44 p-3 border border-white/10 rounded-lg text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500/20 bg-[#161616] text-white placeholder-gray-600"
          ...
          <div>
            ...
            <textarea
              readOnly
              className="w-full h-44 p-3 border border-white/10 rounded-lg text-xs bg-[#161616] text-white"
              value={output}
...
function RandomWordGenerator({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  ...
  return (
    <div className="space-y-6" id="random-word-container">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-sm text-gray-400">Roll random sets of English vocabulary blocks filtered by Parts of Speech features and character bounds standard limits.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          <h3 className="font-bold text-white border-b border-white/5 pb-2 mb-2 font-sans">Tuning Metrics</h3>
          
          <div className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Words Count:</label>
              <input
                type="number"
                min="1"
                max="100"
                ...
                className="w-full p-2 bg-[#161616] border border-white/10 rounded-lg text-xs font-semibold text-white focus:outline-none"
              />
            </div>

            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Word Category:</label>
              <select
                value={category}
                onChange={(e: any) => setCategory(e.target.value)}
                className="w-full p-2.5 border border-white/10 rounded-lg text-xs bg-[#141414] text-white focus:outline-none"
              >
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="text-xs font-semibold text-gray-400 block mb-1">Min Length:</label>
                <input
                  type="number"
                  min="2"
                  max={maxLength}
                  ...
                  className="w-full p-2 bg-[#161616] border border-white/10 rounded-lg text-xs text-white focus:outline-none"
                />
              </div>
              <div>
                <label className="text-xs font-semibold text-gray-400 block mb-1">Max Length:</label>
                <input
                  type="number"
                  min={minLength}
                  max="20"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Output Separator:</label>
              <select
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <textarea
            readOnly
            className="w-full h-80 p-4 border border-white/10 rounded-lg text-sm font-mono bg-[#161616] text-white focus:outline-none placeholder-gray-650 resize-y"
            value={resultingText}
...
function PrivacyPolicyGenerator({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  ...
  return (
    <div className="space-y-6" id="privacy-policy-container">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-sm text-gray-400">Formulate a professional Privacy Policy layout tailored for General Data Protection Regulation (GDPR) and California Consumer Privacy Act (CCPA) compliance.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          <h3 className="font-bold text-white border-b border-white/5 pb-2 mb-2 font-sans">Company Details</h3>
          
          <div className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Company / Team Name*:</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Website Name:</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Website URL*:</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Contact Email Link:</label>
              <input
                type="text"
            ...
            <div className="space-y-2 pt-2 border-t border-white/5">
              <label className="flex items-center gap-2 cursor-pointer font-medium text-gray-305 hover:text-white">
                <input
                  type="checkbox"
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <textarea
            readOnly
            className="w-full h-96 p-4 border border-white/10 rounded-lg text-xs font-mono bg-[#161616] text-white focus:outline-none placeholder-gray-650 resize-y opacity-90"
            value={policyOutput}
...
function TermsConditionsGenerator({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  const [formData, setFormData] = useState({
    companyName: '',
    websiteUrl: '',
  ...
  return (
    <div className="space-y-6" id="terms-conditions-container">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-sm text-gray-400">Compile trademark-safe Term of Service policy agreements defining intellectual property rules and legal boundaries offline in-browser.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          <h3 className="font-bold text-white border-b border-white/5 pb-2 mb-2 font-sans">Policy Settings</h3>
          
          <div className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Company / App Name*:</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Governing Country:</label>
              <input
                type="text"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">State / Province (Jurisdiction):</label>
              <input
                type="text"
...
function DisclaimerGenerator({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  const [formData, setFormData] = useState({
    companyName: '',
    websiteUrl: ''
  });
  ...
  return (
    <div className="space-y-6" id="disclaimer-gen-container">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-sm text-gray-400">Generate structured liability disclaimers for professional advice limitations, affiliate disclosures, or external linking risks.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          <h3 className="font-bold text-white border-b border-white/5 pb-2 mb-2 font-sans">Disclaimer Metrics</h3>
          
          <div className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Company / App Owner*:</label>
              <input
                type="text"
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <textarea
            readOnly
            className="w-full h-80 p-4 border border-white/10 rounded-lg text-xs font-mono bg-[#161616] text-white focus:outline-none placeholder-gray-650 resize-y opacity-90"
...
// 11. TEXT REPEATER
function TextRepeater({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  const [input, setInput] = useState('');
  const [count, setCount] = useState(10);
  ...
  return (
    <div className="space-y-6" id="text-repeater-container">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-sm text-gray-400">Instantly repeat lines of keywords, phrases, or emojis with personalized layout delimiters for debugging or assets creation.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          <h3 className="font-bold text-white border-b border-white/5 pb-2 mb-2 font-sans">Settings</h3>
          
          <div className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Repeat Count:</label>
              <input
                type="number"
                min="1"
                max="1000"
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Delimiter Separator:</label>
              <select
                value={separator}
            ...
            {separator === 'custom' && (
              <div>
                <label className="text-xs font-semibold text-gray-400 block mb-1">Custom Delimiter:</label>
                <input
                  type="text"
                  value={customChar}
                  onChange={(e) => setCustomChar(e.target.value)}
            ...
            {outputLength > 0 && (
              <div className="pt-2 border-t border-white/5 space-y-1 text-xs">
                <span className="text-gray-450 block text-gray-400 font-mono">Stream Metrics:</span>
        ...
        <div className="lg:col-span-2 space-y-4">
          <div>
            <label className="text-xs font-mono text-gray-400 mb-1 block">TEXT/EMOJI TO REPEAT:</label>
            <input
              type="text"
              placeholder="e.g. Sparkles ✨"
              value={input}
              onChange={(e) => setInput(e.target.value)}
              className="w-full p-3 border border-white/10 rounded-lg text-sm bg-[#161616] text-white focus:outline-none"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              readOnly
              className="w-full h-56 p-3 border border-white/10 rounded-lg text-xs font-mono bg-[#161616] text-white opacity-95"
              value={output}
...
// 12. TEXT SORTER
function TextSorter({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  const [input, setInput] = useState('');
  ...
  return (
    <div className="space-y-6" id="text-sorter-container">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-sm text-gray-400">Order, reverse, or randomize tabular list entries vertical vectors or CSV lists inside browser context safely.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          <h3 className="font-bold text-white border-b border-white/5 pb-2 mb-2 font-sans">Sorting Controls</h3>
          
          <div className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Sorting Order:</label>
              <select
                value={sortOrder}
                onChange={(e: any) => setSortOrder(e.target.value)}
                className="w-full p-2.5 border border-white/10 rounded-lg text-xs bg-[#141414] text-white focus:outline-none font-semibold"
              >
            ...
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Entry Separator:</label>
              <select
                value={separator}
                onChange={(e: any) => setSeparator(e.target.value)}
                className="w-full p-2.5 border border-white/10 rounded-lg text-xs bg-[#141414] text-white focus:outline-none font-semibold"
        ...
        <div className="lg:col-span-2 space-y-4">
          <div>
            <label className="text-xs font-mono text-gray-400 mb-1 block">SOURCE LIST:</label>
            <textarea
              className="w-full h-40 p-3 border border-white/10 rounded-lg text-xs font-mono focus:outline-none focus:ring-1 focus:ring-indigo-500/20 bg-[#161616] text-white placeholder-gray-600"
          ...
          <div>
            ...
            <textarea
              readOnly
              className="w-full h-40 p-3 border border-white/10 rounded-lg text-xs font-mono bg-[#161616] text-white"
              value={output}
...
// 13. COMMA SEPARATOR
function CommaSeparator({ onCopy, copiedStatus }: { onCopy: (text: string, id: string) => void; copiedStatus: string | null }) {
  const [input, setInput] = useState('');
  ...
  return (
    <div className="space-y-6" id="comma-separator-container">
      <div className="pb-4 border-b border-white/5">
        ...
        <p className="text-sm text-gray-400">Interchange vertical single column metrics with horizontal comma-delimited CSV strings instantly.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1 space-y-4 bg-[#141414] border border-white/5 p-5 rounded-xl text-sm">
          ...
          <div className="space-y-3">
            <div>
              <label className="text-xs font-semibold text-gray-400 block mb-1">Conversion Direction:</label>
              <select
                value={direction}
            ...
            <label className="flex items-center gap-2 cursor-pointer font-medium text-gray-305 hover:text-white pt-2 border-t border-white/5">
              <input
                type="checkbox"
                checked={stripQuotes}
            ...
            <label className="flex items-center gap-2 cursor-pointer font-medium text-gray-350 hover:text-white">
              <input
                type="checkbox"
                checked={trimValues}
        ...
        <div className="lg:col-span-2 space-y-4">
          <div>
            <label className="text-xs font-mono text-gray-400 mb-1 block">INPUT STREAM:</label>
            <textarea
              className="w-full h-40 p-3 border border-white/10 rounded-lg text-xs font-mono focus:outline-none focus:ring-1 focus:ring-indigo-500/20 bg-[#161616] text-white placeholder-gray-600"
